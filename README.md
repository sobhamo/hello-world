...
## ThingPlug를 위한 Device 미들웨어 설치 및 실행가이드(Beaglebone Black)
...
* **Noise** - 센서 이름 : [FC-04], 종류 : 소리 감지 센서, 특징 : 소리 감지 센서로 현재 개발 환경에서 GPIO 51번에 연결되며, 평소에는 0값을 리턴하고, 특정 크기 이상의 소리를 감지하면 1값을 리턴한다. 아래 그림의 빨간 부분을 조절하여 감지 범위를 설정할 수 있다. 
![](images/FC-042.jpg)
